...
function describeContents({contents, opening = 'a ', closing = '', level = 0}) {
  ...
  if (contents.length) {
    ...
    for (const {object, contents: itemContents} of contents) {
...
const responses = {
  ...
  steps: {
    ...
    move(action) {
      ...
      if (code === 'sm-nd') {
        let directions = {
          n: 'north',
          s: 'south',
          e: 'east', // eslint-disable-line unicorn/prevent-abbreviations
          w: 'west',
          u: 'up',
          d: 'down',
          ne: 'northeast',
    ...
    put(action) {
      ...
      // console.log(JSON.stringify(action.steps, null, 2))
      // console.log(JSON.stringify(action.steps.put, null, 2))
